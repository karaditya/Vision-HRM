...
class VisionPretrainConfig(pydantic.BaseModel):
    # Config
    arch: ArchConfig
    # Data
    data_path: str
...
class NPYPuzzleDataset(Dataset):
    def __init__(self, data_dir: str, image_size: int = 32, patch_size: int = 4, num_channels: int = 3):
        self.inputs = np.load(os.path.join(data_dir, "all__inputs.npy"))
        self.labels = np.load(os.path.join(data_dir, "all__labels.npy"))

        self.num_patches_per_image = (image_size // patch_size) ** 2
        self.patch_dim = patch_size * patch_size * num_channels

        assert self.inputs.shape[1] == self.patch_dim, "Patch dimension mismatch"
        assert len(self.inputs) % self.num_patches_per_image == 0, "Dataset not divisible into full images"

        self.num_images = len(self.inputs) // self.num_patches_per_image

    def __len__(self):
        return self.num_images

    def __getitem__(self, idx):
        start = idx * self.num_patches_per_image
        end = start + self.num_patches_per_image
        x = torch.from_numpy(self.inputs[start:end]).float()  # [num_patches, patch_dim]
        y = torch.tensor(self.labels[start], dtype=torch.long)  # all patches share same label
        return {"inputs": x, "labels": y}


def create_dataloader(config, split: str, test_set_mode: bool, epochs_per_iter: int,
                      global_batch_size: int, rank: int, world_size: int):

    # Locate split directory
    data_dir = os.path.join(config.data_root, split)

    # Load dataset
    dataset = NPYPuzzleDataset(
        data_dir,
    ...
    if world_size > 1:
        sampler = DistributedSampler(dataset, num_replicas=world_size, rank=rank, shuffle=not test_set_mode)
    else:
        sampler = None
    ...
    dataloader = DataLoader(
        ...
        sampler=sampler,
        shuffle=(sampler is None and not test_set_mode),
        num_workers=getattr(config, "num_workers", 2),
        pin_memory=True,
        drop_last=True
    )

    # Build metadata compatible with init_train_state
    metadata = PuzzleDatasetMetadata(
        num_samples=len(dataset),
        total_groups=len(dataset),  # one group = one image
        mean_puzzle_examples=dataset.num_patches_per_image,
        num_patches_per_image=dataset.num_patches_per_image,
        patch_dim=dataset.patch_dim,
        split=split,
    )

    def iterator():
        for batch in dataloader:
            yield split, batch, global_batch_size

    return iterator(), metadata


# def create_dataloader(config: VisionPretrainConfig, split: str, rank: int, world_size: int, **kwargs):
...
def init_train_state(config: VisionPretrainConfig, train_metadata: PuzzleDatasetMetadata, world_size: int):
...
def save_train_state(config: VisionPretrainConfig, train_state: TrainState):
...
def compute_lr(base_lr: float, config: VisionPretrainConfig, train_state: TrainState):
...
def train_batch(config: VisionPretrainConfig, train_state: TrainState, batch: Any, global_batch_size: int, rank: int, world_size: int):
    train_state.step += 1
...
def evaluate(config: VisionPretrainConfig, train_state: TrainState, eval_loader: torch.utils.data.DataLoader, eval_metadata: PuzzleDatasetMetadata, rank: int, world_size: int):
...
def load_synced_config(hydra_config: DictConfig, rank: int, world_size: int) -> VisionPretrainConfig:
    objects = [None]
    if rank == 0:
        config = VisionPretrainConfig(**hydra_config)  # type: ignore

        # Naming
        if config.project_name is None:
            config.project_name = f"CIFAR Vision HRM"
